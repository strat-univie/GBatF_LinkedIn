...
LI_CLIENT_ID = LINKEDIN.get("client_id")
LI_CLIENT_SECRET = LINKEDIN.get("client_secret")
LI_REDIRECT_URI = LINKEDIN.get("redirect_uri")
OIDC_SCOPE = "openid profile email"

# App state signing (use your own secret if set)
APP_STATE_SECRET = st.secrets.get("APP_STATE_SECRET", LI_CLIENT_SECRET or "dev-secret")
...
if not (LI_CLIENT_ID and LI_CLIENT_SECRET and LI_REDIRECT_URI):
...
def make_signed_state(secret: str, ttl_seconds: int = 900) -> str:
    payload = {"ts": int(time.time()), "rnd": _b64url_encode(hashlib.sha256(str(time.time()).encode("utf-8")).digest()[:16]).strip("=")}
...
def persist_signin_row(userinfo: dict):
    ...
    try:
        row = [
            ...
            _stringify_locale(userinfo.get("locale")),
        ]
...
if qp_code and not st.session_state["li_authed"]:
    if qp_state and verify_signed_state(APP_STATE_SECRET, qp_state, max_age_seconds=900):
        try:
            ...
            # Show the exception type & message (helps diagnose any remaining issues)
            import traceback
            st.error(f"LinkedIn sign-in failed: {type(e).__name__}: {e}")
...
if chat_disabled:
    ...
    with center:
        state_token = make_signed_state(APP_STATE_SECRET)
