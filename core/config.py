...
def get_settings() -> Settings:
    ...
    if not VSTORE:
        st.error("Missing OPENAI_VECTOR_STORE_ID (required for file_search)."); st.stop()
    if not (LI.get("client_id") and LI.get("client_secret") and LI.get("redirect_uri")):
        st.error("LinkedIn OIDC not configured (client_id, client_secret, redirect_uri)."); st.stop()
